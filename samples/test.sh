#!/usr/bin/env bash
# ==============================================================================
# RouteWarden Live Testing Script (Caddy v2)
...
wait_for_caddy() {
    log_info "Waiting for Caddy to be ready on port 8080..."
    local retries=30
    while ! curl -s -f "http://${BASE_HOST}:8080/safe" > /dev/null 2>&1; do
        sleep 1
        retries=$((retries - 1))
        if [ "$retries" -le 0 ]; then
            echo -e "${RED}[ERROR] Caddy did not become ready in time.${NC}"
            exit 1
        fi
    done
    log_info "Caddy is responding. Starting test suite!"
}
...
# ------------------------------------------------------------------------------
# 6. Header Inspection (check_headers, Port 8094)
# ------------------------------------------------------------------------------
test_header_inspection() {
    ...
    # Unchecked header with sensitive path should NOT be blocked by check_headers
...
test_security_logging() {
    ...
    if command -v docker > /dev/null 2>&1 && docker compose ps --status running | grep -q "caddy"; then
        local logs
        logs=$(docker compose logs --tail=50 caddy 2>/dev/null || true)
        ...
        # Check for structured Zap log
        if echo "$logs" | grep -qi "routewarden_block" && echo "$logs" | grep -qi "path_blocked"; then
            log_pass "Structured Zap security event logged with reason 'path_blocked'"
        else
            log_fail "Zap security event" "routewarden_block with reason path_blocked" "$logs"
        fi
    else
...
main() {
    echo -e "${CYAN}======================================================================${NC}"
    echo -e "${CYAN}       RouteWarden for Caddy — Live Test & Mode Verification Suite      ${NC}"
    echo -e "${CYAN}======================================================================${NC}"
